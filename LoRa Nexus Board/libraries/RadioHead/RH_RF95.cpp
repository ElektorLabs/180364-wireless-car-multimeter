...
void RH_RF95::handleInterrupt()
{
    ...
    {
    printf("IRQ Flags 0x%02x",irq_flags);
...
bool RH_RF95::available()
{
    ...
    // Read the interrupt register
    uint8_t irq_flags = spiRead(RH_RF95_REG_12_IRQ_FLAGS);
    /* We also check if we are in the right modem mode or if we had a modem reboot */
    if( 0== (spiRead(RH_RF95_REG_01_OP_MODE)&0x80) ){
        /* we are no longer in lora mode and need to do a reinit */
        printf("Modem restart detected need reinit");
        printRegisters();
        return false;
        
    }
    
    if ( (_mode == RHModeRx) && ( 0 != (  irq_flags & RH_RF95_RX_DONE ) ))
    {
    ...
    spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff); // Clear all IRQ flags
    /* We do it twice as in the original irq routine */
    spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff); // Clear all IRQ flags
    /* make sure the write did succseed */
    if( 0 != spiRead(RH_RF95_REG_12_IRQ_FLAGS) ){
        printf("IRQ Clear fault\n");
        printRegisters();
    } else {
